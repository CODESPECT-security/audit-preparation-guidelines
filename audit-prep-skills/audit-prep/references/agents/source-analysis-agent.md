...
### Check 11: tx.origin authorization
Pattern: `tx\.origin`
For each match, read 3 lines of context. Flag if used in a `require(`, `if (`, or assignment that gates access control.
Do NOT flag legitimate uses: EIP-712 `ecrecover` context, pure comparison for informational purposes.
Deduction: -15 if used for authorization (replace with `msg.sender`)

### Check 12: Unjustified unchecked blocks
Pattern: `unchecked\s*\{`
For each match, read 3 lines above. If no comment explains why overflow/underflow cannot occur, flag it.
Deduction: -5 each (cap -20)
Every unchecked block is a promise to the auditor. If it's not documented, the auditor must prove it themselves.
