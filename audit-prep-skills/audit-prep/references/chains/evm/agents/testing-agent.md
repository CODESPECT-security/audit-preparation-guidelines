...
| Integration | files matching `*Integration*\|*E2E*` | test/ |
| Fork tests | files matching `*Fork*` OR pattern `createFork\|vm\.createFork\|RPC_URL` | test/ |
| Fuzz/invariant | `testFuzz_\|test_Fuzz\|invariant_\|fuzz` | test/ |
| Fuzz run count | `\[fuzz\]` section with `runs\s*=\s*\d+` | foundry.toml |
...
| Fork tests | None found and protocol uses external state (oracles, DeFi) | -10 |
| Fuzz run count | `runs` configured < 1000 (explicitly lower than recommended) | -5 |
...
Fork test deduction only applies when the protocol clearly depends on mainnet state. If no `latestRoundData`, `IUniswapV2`, or similar external references are found in source files, skip the fork deduction.
